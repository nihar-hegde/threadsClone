...
export async function fetchUsers({
  ...
  try {
    ...
    throw new Error(`Error while fetching all usres: ${error.message}`)
  }
}
...
export async function getActivity(userId:string) {
  try {
    ...
    //*collect all the child threads ids (replies) from the child fields (basically collect all the comments on origial post)
    const childThreadIds = userThreads.reduce((acc,userThread)=>{
      return acc.concat(userThread.children)
    },[])
    //?get all replies that are made by the same users
    //!so that we can ignore the replies by the same user
    ...
    throw new Error(`Failed while fetching activitiees: ${error.message}`)
  }
